...
func ExportBlocks(dbPath string, outputPath string) (flow.Identifier, error) {
	...
	hdrs, err := headers.FindHeaders(func(_ *flow.Header) bool { return true })
	if err != nil {
		return flow.Identifier{}, fmt.Errorf("issue traversing headers: %w", err)
	}

	var lastParentBlockID flow.Identifier
	maxHeight := uint64(0)

	for _, h := range hdrs {
		if h.Height > maxHeight {
			maxHeight = h.Height
			lastParentBlockID = h.ParentID
		}
	}
	if err != nil {
		return flow.Identifier{}, fmt.Errorf("issue find the last block: %w", err)
	}
	fmt.Println(">>> last block", maxHeight, lastParentBlockID.String())

	activeBlockID := lastParentBlockID
	outputFile := filepath.Join(outputPath, "blocks.jsonl")

	fi, err := os.Create(outputFile)
	if err != nil {
		return flow.Identifier{}, fmt.Errorf("could not create block output file %w", err)
	}
	defer fi.Close()
	...
	for {
		...
		if err != nil {
			return flow.Identifier{}, fmt.Errorf("could not create a json obj for a block: %w", err)
		}
		_, err = blockWriter.WriteString(string(jsonData) + "\n")
		if err != nil {
			return flow.Identifier{}, fmt.Errorf("could not write block json to the file: %w", err)
		}
		blockWriter.Flush()
